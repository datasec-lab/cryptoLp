...
void test_K_medians(uint64_t *x) {
  uint64_t comm_start = math->iopack->get_comm();
  uint64_t *y = new uint64_t[K];
  
  INIT_TIMER;
  START_TIMER;
  math->K_medians(MAX_ITER, dim, x, y, bw_x, K);
  STOP_TIMER("K-medians:");
  uint64_t comm_end = math->iopack->get_comm();
  double comm = (comm_end - comm_start) / (1.0 * (1ULL << 20));
  cout << "Total Communication for K-medians: " << comm << " mb" << endl;

  if (party == ALICE) {
    iopack->io->send_data(x, dim * sizeof(uint64_t));
    iopack->io->send_data(y, K * sizeof(uint64_t));
  } else { // party == BOB
    uint64_t *x0 = new uint64_t[dim];
    uint64_t *y0 = new uint64_t[K];
    iopack->io->recv_data(x0, dim * sizeof(uint64_t));
    iopack->io->recv_data(y0, K * sizeof(uint64_t));

    for (int i = 0; i < dim; i++) {
      cout << "x[" << i << "] = " << signed_val(x[i] + x0[i], bw_x) << endl;
    }
    
    cout << "<><><><><><><><><><><><><><><><><><><><><><><><><><><><>" << endl;
    cout << "K-medians Results:" << endl;
    cout << "<><><><><><><><><><><><><><><><><><><><><><><><><><><><>" << endl;

    for (int i = 0; i < K; i++) {
      cout << "y[" << i << "] = " << signed_val(y[i] + y0[i], bw_x) << endl;
    }
