...
void test_sort(uint64_t *x) {
  ...
  INIT_TIMER;
  START_TIMER;
  // math->sort_naive(x, dim, bw_x);
  math->sort_bitonic(x, dim, bw_x);
  STOP_TIMER("Sort:");
  uint64_t comm_end = math->iopack->get_comm();
  double comm = (comm_end - comm_start) / (1.0 * (1ULL << 20));
  cout << "Total Communication for sorting: " << comm << " mb" << endl;

  if (party == ALICE) {
    iopack->io->send_data(x, dim * sizeof(uint64_t));
    iopack->io->send_data(temp_x, dim * sizeof(uint64_t));
  } else { // party == BOB
    uint64_t *x0 = new uint64_t[dim];
    uint64_t *temp_x0 = new uint64_t[dim];
    iopack->io->recv_data(x0, dim * sizeof(uint64_t));
    iopack->io->recv_data(temp_x0, dim * sizeof(uint64_t));

    uint64_t *x_sorted = new uint64_t[dim];
    uint64_t *temp = new uint64_t[dim];
    for (int i = 0; i < dim; i++) {
      x_sorted[i] = (x[i] + x0[i]) & mask_x;
      temp[i] = (temp_x[i] + temp_x0[i]) & mask_x;
    } 
